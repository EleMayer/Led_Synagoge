...
.srow .s-name{color:var(--fg)}
.srow .s-f{color:var(--muted);font-size:13px;text-align:right;white-space:nowrap}
.srow input[type=number]{width:54px;padding:7px 6px;border-radius:8px;
  border:1px solid var(--line-strong);background:var(--input-bg);color:var(--fg);
...
<section class="card" id="autoCard" style="display:none">
  ...
  <p class="hint" data-i18n="auto.editHint">Uhrzeiten und Helligkeiten des Tagesprofils. Ab „Nacht" bleibt die Beleuchtung aus.</p>

  <div class="sched">
    <div class="srow">
      <span class="s-name" data-i18n="phase.morning">Morgen</span>
      <span class="s-f"><input type="number" id="tMorning" min="0" max="23" inputmode="numeric"> <span data-i18n="unit.hour">Uhr</span></span>
      <span class="s-f">&rarr; <input type="number" id="bMorning" min="0" max="100" inputmode="numeric"> %</span>
    </div>
    <div class="srow">
      <span class="s-name" data-i18n="phase.day">Tag</span>
      <span class="s-f"><input type="number" id="tDay" min="0" max="23" inputmode="numeric"> <span data-i18n="unit.hour">Uhr</span></span>
      <span class="s-f"><input type="number" id="bDay" min="0" max="100" inputmode="numeric"> %</span>
    </div>
    <div class="srow">
      <span class="s-name" data-i18n="phase.evening">Abend</span>
      <span class="s-f"><input type="number" id="tEvening" min="0" max="23" inputmode="numeric"> <span data-i18n="unit.hour">Uhr</span></span>
      <span class="s-f"><input type="number" id="bEveStart" min="0" max="100" inputmode="numeric"> % &rarr; <input type="number" id="bEveEnd" min="0" max="100" inputmode="numeric"> %</span>
    </div>
    <div class="srow">
      <span class="s-name" data-i18n="phase.night">Nacht</span>
      <span class="s-f"><input type="number" id="tNight" min="0" max="23" inputmode="numeric"> <span data-i18n="unit.hour">Uhr</span></span>
      <span class="s-f" data-i18n="phase.off">aus</span>
    </div>
  </div>

  <p class="hint err" id="schedError" style="display:none"></p>
  <button class="primary" id="schedSave" onclick="saveSchedule()" data-i18n="auto.save">Profil speichern</button>
...
const T = {
  de:{
    ...
    "auto.editHint":"Uhrzeiten und Helligkeiten des Tagesprofils. Ab „Nacht“ bleibt die Beleuchtung aus.",
    "auto.save":"Profil speichern","unit.hour":"Uhr",
    "sched.errOrder":"Uhrzeiten müssen aufsteigend sein (Morgen < Tag < Abend < Nacht, 0–23).",
    "sched.errRange":"Bitte gültige Werte eingeben (Stunden 0–23, Helligkeit 0–100 %).",
  ...
  en:{
    ...
    "auto.editHint":"Times and brightness of the daily profile. From “Night” the lighting stays off.",
    "auto.save":"Save profile","unit.hour":"h",
    "sched.errOrder":"Times must be ascending (morning < day < evening < night, 0–23).",
    "sched.errRange":"Please enter valid values (hours 0–23, brightness 0–100 %).",
...
function updateUI(data)
{
    ...
    if(data.sched)
    {
        // Automatik-Profil (Uhrzeiten und Helligkeiten) vom Controller in die
        // Eingabefelder uebernehmen (gerade bearbeitete Felder bleiben stehen).
        schedule = data.sched;
...
// Letztes vom Controller gemeldetes Automatik-Profil.
let schedule = {
    tMorning: 6, tDay: 8, tEvening: 18, tNight: 23,
    bMorning: 90, bDay: 90, bEveStart: 60, bEveEnd: 25
};

// Alle Profil-Eingabefelder (die IDs entsprechen den Feldnamen im Status).
const schedIds = ["tMorning", "tDay", "tEvening", "tNight",
                  "bMorning", "bDay", "bEveStart", "bEveEnd"];

// Fuellt die Eingabefelder mit dem Profil vom Controller. Ein gerade
// bearbeitetes Feld wird nicht ueberschrieben.
function fillSchedule(s)
{
    for(let i = 0; i < schedIds.length; i++)
    {
        const el = document.getElementById(schedIds[i]);
        if(el && el !== document.activeElement && s[schedIds[i]] !== undefined)
        {
            el.value = s[schedIds[i]];
        }
    }
}

// Liest ein Eingabefeld als ganze Zahl.
function schedNum(id)
{
    return parseInt(document.getElementById(id).value, 10);
}

// Prueft die Eingaben und schickt das Profil an den Controller.
function saveSchedule()
{
    const tM  = schedNum("tMorning");
    const tD  = schedNum("tDay");
    const tE  = schedNum("tEvening");
    const tN  = schedNum("tNight");
    const bM  = schedNum("bMorning");
    const bD  = schedNum("bDay");
    const bES = schedNum("bEveStart");
    const bEE = schedNum("bEveEnd");

    const err = document.getElementById("schedError");
    const vals = [tM, tD, tE, tN, bM, bD, bES, bEE];

    for(let i = 0; i < vals.length; i++)
    {
        if(isNaN(vals[i]))
        {
            err.innerText = t("sched.errRange");
            err.style.display = "";
            return;
        }
    }

    // Uhrzeiten muessen aufsteigend sein: Morgen < Tag < Abend < Nacht.
    if(tM < 0 || tN > 23 || !(tM < tD && tD < tE && tE < tN))
    {
        err.innerText = t("sched.errOrder");
        err.style.display = "";
        return;
    }

    const bs = [bM, bD, bES, bEE];
    ...
    send({ sched: {
        tMorning: tM, tDay: tD, tEvening: tE, tNight: tN,
