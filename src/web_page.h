...
// ============================================================================
//  Bedien-PWA fuer die LED-Fassade.
//  Design uebernommen von led_sample3 (dunkles, cleanes "Klinik"-Design mit
//  wenig Farbe). Funktion wie gehabt: Modus-Wahl, Helligkeitsregler, Szenen,
//  WLAN-Konfiguration, Live-Status - per WebSocket (JSON), Fallback /api/status.
// ============================================================================
...
:root{
  /* Dunkles, cleanes Design mit wenig Farbe (aus led_sample3) */
  --bg:#12171d;
  --card:#1a212a;
  --line:#28323d;
  --line-strong:#374350;
...
/* --- Schieberegler: dunkle Schiene, heller Griff --- */
...
/* --- Tagesverlauf-Vorschau (24h-Helligkeitskurve) --- */
...
<section class="card">
  ...
  <div class="modes">
    ...
    <button class="mode" data-mode="6" onclick="sendMode(6)">Ewiges Licht</button>
    <button class="mode" data-mode="7" onclick="sendMode(7)">Schabbat</button>
    <button class="mode" data-mode="8" onclick="sendMode(8)">Chanukka</button>
    <button class="mode" data-mode="9" onclick="sendMode(9)">Gedenken</button>
  </div>
...
<section class="card">
  ...
  <p class="hint">Fuer Lauflicht, Pulsieren und Atmen. Die thematischen Modi
    (Ewiges Licht, Schabbat, Chanukka, Gedenken) sind fest hinterlegt.</p>
</section>
...
<!-- Zeitprofil (Automatik) - einstellbar -->
<section class="card">
...
const MODE_DESC = {
  ...
  6:"Ewiges Licht (Ner Tamid): stetiges, ruhig atmendes Licht. Nachtabschaltung ab 23:00 wie alle Modi.",
  7:"Schabbat-Kerzen: zwei sanft flackernde Kerzenlichter auf den Segmenten.",
  8:"Chanukka: zunehmende Lichter, wächst in acht Schritten von der Mitte nach außen.",
  9:"Gedenken: sehr gedämpftes Licht, das über Minuten sanft auf- und abschwillt."
};
...
{
    // Den aktiven Modus-Button hervorheben.
    ...
    // Kopfzeile: Modus-Name, Beschreibung, Uhr.
    ...
    // WLAN-Karte: aktuell verbundenes Netz.
    ...
    // Automatik-Zeitprofil in die Eingabefelder uebernehmen.
    if(data.sched)
    {
    ...
    // Tagesverlauf-Vorschau aktualisieren (Werte + "jetzt"-Markierung).
    renderScheduleCurve();
...
// Baut eine Zeile "Beschriftung : Wert" fuer die System-Karte.
...
// Ein Zeitprofil-Feld setzen - aber nicht, waehrend der Benutzer darin tippt.
...
// Liest ein Zahlenfeld und begrenzt es auf min..max.
...
// Liest das Zeitprofil aus den Eingabefeldern in ein Objekt.
...
// Das ganze Zeitprofil an den Controller senden - nur wenn die Uhrzeiten
// sinnvoll aufsteigend sind, sonst waere die Automatik-Kurve luekenhaft.
...
// Automatik-Helligkeit (%) fuer eine Minute des Tages - identisch zur Firmware.
function brightnessAtMinute(m, s)
{
    ...
    if(m >= s.tMorning * 60 && m < s.tDay * 60)
    {
        ...
        return Math.round(p * s.bMorning);         // Morgen: hochdimmen
    }
...
// Zeichnet die 24-Stunden-Helligkeitskurve aus den aktuellen Eingabefeldern.
function renderScheduleCurve()
{
    ...
    // Zeichenflaeche (muss zum viewBox 320x132 passen).
    ...
    // Kurve alle 6 Minuten abtasten (bildet auch Spruenge sauber ab).
    let pts = [];
    ...
    // Waagerechte Hilfslinien + Y-Beschriftung (0 / 50 / 100 %).
...
// Jeden Regler mit seiner Funktion verbinden.
...
// Kurven-Vorschau live aktualisieren, waehrend die Zeitprofil-Felder editiert werden.
